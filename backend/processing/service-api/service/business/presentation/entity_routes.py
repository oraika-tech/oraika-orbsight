...
def get_all_entities(
    ...
    return handler.get_all_entities(user_info.preferred_tenant_id, enabled)
...
def enabled_entities_count(user_info=Depends(get_current_user), handler=Depends(get_handler)):
    ...
    return handler.enabled_entities_count(user_info.preferred_tenant_id)
...
def get_entity(entity_id: UUID, user_info=Depends(get_current_user), handler=Depends(get_handler)):
    ...
    entity = handler.get_entity(user_info.preferred_tenant_id, entity_id)
    if not entity:
...
def update_entity_enable_state(
    ...
    return handler.update_entity_enable_state(user_info.preferred_tenant_id, entity_id, status_request.enabled)
